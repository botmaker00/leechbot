...
# Persistent session cache (no timeout - persistent until restart)
PERSISTENT_STREAMRIP_SESSION_CACHE = {
    'qobuz': None,
    'tidal': None,
    'deezer': None,
    'soundcloud': None
...
# Session cache status tracking (persistent until restart)
PERSISTENT_STREAMRIP_SESSION_STATUS = {
    'qobuz': False,
    'tidal': False,
    'deezer': False,
    'soundcloud': False
...
async def _get_or_create_cached_client(platform_name, config):
    ...
    try:
        ...
        # Check persistent session cache first (no timeout)
        if (PERSISTENT_STREAMRIP_SESSION_CACHE.get(platform_name) is not None and
            PERSISTENT_STREAMRIP_SESSION_STATUS.get(platform_name, False)):
            try:
                cached_client = PERSISTENT_STREAMRIP_SESSION_CACHE[platform_name]
                # Verify client is still valid
                if (hasattr(cached_client, "session") and cached_client.session and
                    not cached_client.session.closed):
                    LOGGER.info(f"🔍 [STREAMRIP-{platform_name.upper()}] Using persistent cached session")
                    record_platform_success(platform_name)
                    return cached_client
                else:
                    # Clear invalid persistent cache
                    LOGGER.info(f"🔍 [STREAMRIP-{platform_name.upper()}] Persistent session invalid, clearing cache")
                    PERSISTENT_STREAMRIP_SESSION_CACHE[platform_name] = None
                    PERSISTENT_STREAMRIP_SESSION_STATUS[platform_name] = False
            except Exception as e:
                LOGGER.info(f"Persistent cache validation failed for {platform_name}: {e}")
                PERSISTENT_STREAMRIP_SESSION_CACHE[platform_name] = None
                PERSISTENT_STREAMRIP_SESSION_STATUS[platform_name] = False

        # Check if we have a valid cached client (fallback to temporary cache)
        if cache_key in STREAMRIP_CLIENT_CACHE:
            cached_time, cached_client = STREAMRIP_CLIENT_CACHE[cache_key]
        ...
        try:
            ...
            # Cache the authenticated client persistently (no timeout) and in temporary cache
            PERSISTENT_STREAMRIP_SESSION_CACHE[platform_name] = client
            PERSISTENT_STREAMRIP_SESSION_STATUS[platform_name] = True
            STREAMRIP_CLIENT_CACHE[cache_key] = (current_time, client)
            _update_client_activity(platform_name)
            record_platform_success(platform_name)  # Reset circuit breaker on success
            LOGGER.info(f"🔍 [STREAMRIP-{platform_name.upper()}] Session cached persistently (no timeout)")
...
async def inline_streamrip_search(_, inline_query: InlineQuery):
    ...
    try:
        ...
        for i, result in enumerate(
            ...
            # Add quality information based on platform and actual account capabilities
            quality_info = ""
            platform = result.get("platform", "").lower()

            # Get actual quality based on account capabilities
            try:
                # Check if we have a cached client for this platform
                if (PERSISTENT_STREAMRIP_SESSION_CACHE.get(platform) is not None and
                    PERSISTENT_STREAMRIP_SESSION_STATUS.get(platform, False)):
                    client = PERSISTENT_STREAMRIP_SESSION_CACHE[platform]

                    # Get maximum quality for this account
                    if hasattr(client, 'max_quality'):
                        max_quality = client.max_quality

                        # Map Streamrip quality levels to descriptions
                        # Streamrip uses: QobuzClient.max_quality=4, TidalClient.max_quality=3, DeezerClient.max_quality=2
                        if platform == "qobuz":
                            if max_quality >= 4:  # Streamrip max for Qobuz
                                quality_info = "Hi-Res FLAC (up to 24-bit/192kHz)"
                            elif max_quality >= 3:
                                quality_info = "Hi-Res FLAC (24-bit/96kHz)"
                            elif max_quality >= 2:
                                quality_info = "CD FLAC (16-bit/44.1kHz)"
                            else:
                                quality_info = "MP3 (320kbps)"
                        elif platform == "tidal":
                            if max_quality >= 3:  # Streamrip max for Tidal
                                quality_info = "Hi-Res FLAC/MQA (up to 24-bit/192kHz)"
                            elif max_quality >= 2:
                                quality_info = "Lossless FLAC (16-bit/44.1kHz)"
                            else:
                                quality_info = "Hi-Fi (320kbps)"
                        elif platform == "deezer":
                            if max_quality >= 2:  # Streamrip max for Deezer
                                quality_info = "FLAC (16-bit/44.1kHz)"
                            else:
                                quality_info = "MP3 (320kbps)"
                        elif platform == "soundcloud":
                            quality_info = "MP3 (320kbps)"
                        else:
                            quality_info = f"Quality Level {max_quality}"
                    else:
                        # Fallback to default mapping
                        quality_mapping = {
                            "qobuz": "Hi-Res FLAC (24-bit/192kHz)",
                            "tidal": "MQA/Hi-Fi (24-bit/96kHz)",
                            "deezer": "FLAC (16-bit/44.1kHz)",
                            "soundcloud": "MP3 (320kbps)"
                        }
                        quality_info = quality_mapping.get(platform, "High Quality")
                else:
                    # No cached client, use conservative estimates
                    quality_mapping = {
                        "qobuz": "FLAC (varies by account)",
                        "tidal": "Hi-Fi (varies by account)",
                        "deezer": "MP3/FLAC (varies by account)",
                        "soundcloud": "MP3 (320kbps)"
                    }
                    quality_info = quality_mapping.get(platform, "High Quality")

            except Exception as e:
                LOGGER.info(f"Could not detect {platform} account quality: {e}")
                # Fallback to default mapping
                quality_mapping = {
                    "qobuz": "Hi-Res FLAC (24-bit/192kHz)",
                    "tidal": "MQA/Hi-Fi (24-bit/96kHz)",
            ...
            description = f"{type_emoji} {media_type_name} • {quality_info}"

            # Add duration for tracks
            if result.get("duration") and result.get("type") in ["track", "song"]:
                minutes = result["duration"] // 60
                seconds = result["duration"] % 60
                description += f" • {minutes}:{seconds:02d}"

            # Add track count for albums/playlists
            elif result.get("track_count") and result.get("type") in ["album", "playlist"]:
                track_count = result["track_count"]
                if result.get("duration"):
                    total_minutes = result["duration"] // 60
                    description += f" • {track_count} tracks • {total_minutes}min"
                else:
                    description += f" • {track_count} tracks"

            # Add lyrics indicator if available
            if result.get("has_lyrics", False):
                description += " • 🎤"
