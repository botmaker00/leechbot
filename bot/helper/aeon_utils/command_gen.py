...
async def get_metadata_cmd(
    ...
    # Validate that streams is a list of dictionaries
    if streams is not None and not isinstance(streams, list):
        LOGGER.error(f"get_streams returned unexpected type: {type(streams)}, expected list")
        return None, None

    if streams and not all(isinstance(stream, dict) for stream in streams):
        LOGGER.error(f"get_streams returned list with non-dict elements: {[type(s) for s in streams]}")
        # Filter out non-dict elements
        streams = [s for s in streams if isinstance(s, dict)]
        if not streams:
            LOGGER.error(f"No valid stream dictionaries found after filtering")
            return None, None
    ...
    for stream in streams:
        # Safety check: ensure stream is a dictionary
        if not isinstance(stream, dict):
            LOGGER.error(f"Expected dict but got {type(stream)}: {stream}")
            continue
